...
export default function ToggleBPage() {
    ...
    <div className="min-h-screen py-12">
      <div className="mx-auto max-w-7xl px-4">
        ...
        <div className="mb-8 text-center">
          ...
          <h1 className="text-2xl font-bold">Toggle B: Switch Style</h1>
          <p className="text-gray-400 mt-2">Sliding toggle switch with labels • ~50% smaller</p>
        ...
        <div className="mx-auto max-w-md">
          <div className="rounded-2xl border border-gray-800 bg-gray-900 p-6">
            ...
            <div className="mb-4 flex items-center justify-between">
              ...
              {/* TOGGLE B: Switch style */}
              <button
                onClick={() => setMode(mode === 'preview' ? 'execute' : 'preview')}
                className="flex items-center gap-2 text-sm"
              >
                <span className={mode === 'preview' ? 'text-amber-400' : 'text-gray-500'}>
                  Preview
                </span>
                <div className={`relative h-5 w-9 rounded-full transition-colors ${
                  mode === 'preview' ? 'bg-amber-600' : 'bg-green-600'
                }`}>
                  <div className={`absolute top-0.5 h-4 w-4 rounded-full bg-white transition-transform ${
                    mode === 'preview' ? 'left-0.5' : 'left-4'
                  }`} />
                </div>
                <span className={mode === 'execute' ? 'text-green-400' : 'text-gray-500'}>
                  Execute
                </span>
              </button>
            </div>
        ...
        <div className="mt-12 mx-auto max-w-2xl">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <div className="rounded-xl border border-purple-500/30 bg-purple-500/5 p-4">
              <p className="text-xs text-purple-400 mb-2">Option B (compact)</p>
              <div className="flex items-center gap-2 text-sm">
                <span className={mode === 'preview' ? 'text-amber-400' : 'text-gray-500'}>Preview</span>
                <div className={`relative h-5 w-9 rounded-full ${mode === 'preview' ? 'bg-amber-600' : 'bg-green-600'}`}>
                  <div className={`absolute top-0.5 h-4 w-4 rounded-full bg-white ${mode === 'preview' ? 'left-0.5' : 'left-4'}`} />
                </div>
                <span className={mode === 'execute' ? 'text-green-400' : 'text-gray-500'}>Execute</span>
              </div>
            </div>
          </div>
        </div>
